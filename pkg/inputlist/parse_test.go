...
func TestParseListCIDR(t *testing.T) {
	numErrors := 3
	numAddrs := 264
	lines := `12:3456:78:90ab:cd:ef01:23:30/125
	192.168.2.0/24
	192.168.4
...
func TestParseListMaxAddresses(t *testing.T) {
	...
	*.domain.com
	...
	r := strings.NewReader(lines)
	addr, errs := ParseList(r, 4)
	if addr != nil {
		t.Fatalf("expected nil addr due to max address")
	}
	...
	r = strings.NewReader(lines)
	addr, errs = ParseList(r, 256)
	if addr != nil {
		t.Fatalf("expected nil addr due to max address with cidr")
	}
