...
// Statements hold queries necessary for the ladonrolemanager
...
// GetStatements populates the statements structure with our queries
func GetStatements(driverName string) *Statements {
	...
	case "postgres", "pgx", "pg":
		...
		stmts.QueryFindByMember = "SELECT organization_id,role_id from am.ladon_role_member WHERE organization_id=$1 and member_id=$2 GROUP BY role_id ORDER BY role_id LIMIT $3 OFFSET $4"
