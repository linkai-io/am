...
func TestBuildGetFilterQuery(t *testing.T) {
	...
	expected := "SELECT organization_id,  address_id,  scan_group_id,  host_address, ip_address,  discovered_timestamp,  (select discovered_by from am.scan_address_discovered_by where discovery_id=sga.discovery_id), last_scanned_timestamp, last_seen_timestamp, confidence_score, user_confidence_score, is_soa, is_wildcard_zone, is_hosted_service, ignored, found_from, ns_record, address_hash FROM am.scan_group_addresses as sga WHERE sga.organization_id = $1 AND sga.scan_group_id = $2 AND sga.address_id > $3 LIMIT 1000"
	if query != expected {
		t.Fatalf("expected:\n%v\ngot:\n%v\n", expected, query)
	}
