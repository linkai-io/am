...
var queryMap = map[string]string{
	...
	"scanGroupVersion":       "select organization_id, scan_group_id, version_name, creation_time, created_by, configuration, config_version, deleted from am.scan_group_versions where organization_id=$1 and scan_group_id=$2 and deleted=false",
	"scanGroupVersionByID":   "select organization_id, scan_group_id, version_name, creation_time, created_by, configuration, config_version, deleted from am.scan_group_versions where organization_id=$1 and scan_group_id=$2 and scan_group_version_id=$3 and deleted=false",
	"scanGroupVersionByName": "select organization_id, scan_group_id, version_name, creation_time, created_by, configuration, config_version, deleted from am.scan_group_versions where organization_id=$1 and scan_group_id=$2 and version_name=$3 and deleted=false",
